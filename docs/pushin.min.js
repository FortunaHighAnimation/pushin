  ...
  class t {
    ...
    getLayers() {
      ...
      for (let e = 0; e < t.length; e++) {
        ...
        this.layers.push(a), this.setZIndex(a, t.length);
      }
    }
    getInpoints(t, e) {
      var s, n;
    ...
    getSpeed(t, e) {
      ...
      return (
        t.dataset.pushinSpeed
          ...
          : (null === (s = this.layerOptions[e]) || void 0 === s
              ? void 0
              : s.speed) && (n = this.layerOptions[e].speed),
        n || 8
      );
    }
    getBreakpointIndex() {
      const t = this.sceneOptions.breakpoints
        .reverse()
        .findIndex(t => t <= window.innerWidth);
      return -1 === t ? 0 : this.sceneOptions.breakpoints.length - 1 - t;
    }
    setZIndex(t, e) {
    ...
    bindEvents() {
      const t = () => {
        (this.scrollPos = window.pageYOffset), this.dolly();
      };
      ...
      const s = t => {
        ...
        (this.scrollPos = Math.max(this.scrollEnd + this.touchStart - e, 0)),
    ...
    isActive(t) {
      const { inpoint: e } = t.params,
        { outpoint: s } = t.params;
      return this.scrollPos >= e && this.scrollPos <= s;
    }
    getInpoint(t) {
    ...
    getScaleValue(t) {
      const e =
        ((this.scrollPos - t.params.inpoint) *
    ...
    setLayerStyle(t) {
      ...
      const s = 0 === t.index,
        ...
        { inpoint: i } = t.params,
        { outpoint: o } = t.params;
      if (s && this.scrollPos < i) e = 1;
      else if (n && this.scrollPos > o) e = 1;
      else if (this.isActive(t)) {
        this.setScale(t.element, this.getScaleValue(t));
        let a =
          Math.max(Math.min(this.scrollPos - i, this.transitionLength), 0) /
          this.transitionLength;
        s && (a = 1);
        let r =
          Math.max(Math.min(o - this.scrollPos, this.transitionLength), 0) /
          this.transitionLength;
        n && (r = 1), (e = Math.min(a, r));
      }
    ...
    showDebugger() {
      ...
      t.classList.add('pushin-debug');
      const e = document.createElement('p');
      (e.innerText = 'Pushin.js Debugger'),
        e.classList.add('pushin-debug__title');
      const s = document.createElement('div');
      s.classList.add('pushin-debug__content'),
        (s.innerText = `Scroll position: ${window.pageYOffset}px`),
        t.appendChild(e),
        t.appendChild(s),
        document.body.appendChild(t),
        window.addEventListener('scroll', () => {
          s.innerText = `Scroll position: ${Math.round(window.pageYOffset)}px`;
        });
    }
  }
  window.pushInStart = e => {
    const s = null != e ? e : {},
